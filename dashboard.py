...
import re
import asyncio
import feedparser # BIBLIOTECA PARA LER NOTÍCIAS
...
# --- 4. FUNÇÃO NOVA: RADAR DE PROMOÇÕES AO VIVO ---
@st.cache_data(ttl=1800) # Cache de 30 minutos para não ficar lento
def buscar_promocoes_live():
    # Lista de Feeds RSS dos principais portais
    feeds = [
    ...
    # Palavras-chave para filtrar (só o que interessa)
    keywords = ["bônus", "transferência", "compra de pontos", "livelo", "esfera", "latam", "smiles", "azul", "tudoazul", "milhas", "100%", "80%"]
    ...
    for feed in feeds:
        try:
            ...
            for entry in d.entries[:15]: # Pega as 15 mais recentes de cada
                titulo = entry.title
                link = entry.link
                data = entry.get("published", "")
                
                # Filtro: Só adiciona se tiver alguma palavra chave no título
                if any(word.lower() in titulo.lower() for word in keywords):
                    # Limpa data se possível
                    try:
                        dt_obj = datetime.strptime(data[0:16], '%a, %d %b %Y')
...
def sistema_logado():
    ...
    # --- PROMOÇÕES (AGORA AO VIVO) ---
    elif menu == "Promoções":
        st.header("🔥 Radar de Promoções (Ao Vivo)")
        if plano == "Free": mostrar_paywall()
        else:
            with st.spinner("Buscando as últimas promoções nos portais..."):
                # Função interna para buscar RSS na hora
                def buscar_promos_live():
                    feeds = [
                        {"url": "https://passageirodeprimeira.com/feed/", "fonte": "Passageiro de Primeira"},
                        {"url": "https://pontospravoar.com/feed/", "fonte": "Pontos pra Voar"},
                        {"url": "https://www.melhoresdestinos.com.br/feed", "fonte": "Melhores Destinos"}
                        ...
                        try:
                            ...
                            for e in d.entries[:8]:
                                if any(k in e.title.lower() for k in keywords):
                                    news.append({"Data": e.published[:16], "Título": e.title, "Fonte": f['fonte'], "Link": e.link})
                        except: pass
                    return pd.DataFrame(news)
                
                df_news = buscar_promos_live()
                ...
                if not df_news.empty:
                    ...
                    st.info("Nenhuma promoção relevante encontrada hoje.")
