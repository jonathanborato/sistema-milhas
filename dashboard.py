...
def sistema_logado():
    ...
    # VERIFICAR PAGAMENTO (STRIPE RETURN)
    ...
    if menu == "Dashboard (Mercado)":
        ...
        if not df_cotacoes.empty:
            ...
            for i, p in enumerate(["Latam", "Smiles", "Azul"]):
                ...
                with cols[i]:
                    st.markdown(f"### {p}")
                    st.markdown(criar_card_preco("🤖 Hotmilhas (Venda)", val_hot, True), unsafe_allow_html=True)
                    st.divider()
        ...
        if plano == "Free": mostrar_paywall()
            ...
            if not dfc.empty:
                ...
                for _, row in dfc.iterrows():
                    ...
                    custo_total += custo; view_data.append({"ID": row['id'], "Programa": row['programa'], "Qtd": f"{qtd:,.0f}".replace(',', '.'), "Custo": formatar_real(custo), "CPM Pago": formatar_real(cpm_pago)})
                st.metric("Total Investido", formatar_real(custo_total))
                st.divider()
